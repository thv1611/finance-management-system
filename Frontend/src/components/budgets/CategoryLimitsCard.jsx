...
export default function CategoryLimitsCard({ budgets = [] }) {
  return (
              ...
              <div key={budget.id || budget.name} className="group">
                <div className="mb-3 flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black text-[#34424d]">{budget.name}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <p className={`mt-1 text-xs font-bold ${over ? "text-[#dd4d58]" : "text-[#8e9aa5]"}`}>
                      {over ? `Overspent by ${formatCurrency(spent - limit)}` : `${Math.round((remaining / Math.max(limit, 1)) * 100)}% Remaining`}
                    </p>
                  </div>
                </div>
