...
export function getBudgetSummary(budgets = []) {
  const totalBudget = budgets.reduce((total, budget) => total + Number(budget.limit || 0), 0);
  const totalSpent = budgets.reduce((total, budget) => total + Number(budget.spent || 0), 0);
  const overspent = budgets.filter((budget) => Number(budget.spent || 0) > Number(budget.limit || 0)).length;
